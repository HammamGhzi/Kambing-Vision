...
export default function AnalisisPage() {
  ...
  async function mulaiAnalisis() {
    ...
    const interval = setInterval(() => { setLoadingMsg(msgs[++i % msgs.length]) }, 1500)

    try {
      const data = await analisisKambing({ imageBase64, mimeType, jenisKambing, cekKurban, cekHarga })
      setHasil(data)
      ...
      await supabase.from('pemeriksaan').insert({
        ...
        hasil_lengkap: data,
        foto_base64: imageBase64.substring(0, 200),
      })
    } catch (err) {
      if (err.isTokenHabis) {
        setTokenHabis(true)
        // Ambil info reset terbaru dari gemini.js
        const usage = getUsageInfo()
        setTokenPesan(`Reset jam ${usage.resetJam}, sekitar ${usage.sisaWaktu}.`)
      } else {
        setError('Gagal menganalisis gambar. Pastikan foto jelas dan API key sudah benar.')
      }
      console.error(err)
    } finally {
      ...
      <div>
        <div className="card">
          ...
          <div className="tab-group">
            ...
            <button className={`tab-btn ${tab === 'camera' ? 'active' : ''}`} onClick={() => switchTab('camera')}>
              Kamera
            </button>
          </div>
          ...
          {tab === 'upload' && (
            <>
              ...
              <input id="file-in" type="file" accept="image/*" style={{ display: 'none' }} onChange={handleFile} />
              {preview && (
                <button className="btn-outline" style={{ marginBottom: 12 }} onClick={() => { setPreview(null); setImageBase64(null); setHasil(null) }}>
                  Ganti foto
                </button>
              )}
            </>
          ...
          {tab === 'camera' && (
            <>
              <div style={{ position: 'relative' }}>
                <video ref={videoRef} className="camera-feed" autoPlay playsInline style={{ display: 'none' }} />
                {cameraActive && (
                  <button
                    onClick={flipCamera}
              ...
              {preview && <img src={preview} alt="Snapshot" className="preview-img" />}

              <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                {!preview && cameraActive && (
                  <button className="btn-outline" style={{ flex: 1 }} onClick={ambilFoto}>
                    Ambil foto
                  </button>
                )}
                {!preview && !cameraActive && !error && (
                  <button className="btn-outline" style={{ flex: 1 }} onClick={() => startCamera()}>
                    Nyalakan kamera
                  </button>
                )}
                {preview && (
                  <button className="btn-outline" style={{ flex: 1 }} onClick={ulangi}>
                    Ulangi
                  </button>
                )}
              </div>
          ...
          <hr style={{ border: 'none', borderTop: '1px solid #f0f0eb', margin: '12px 0' }} />
          <p className="section-label"> Opsi analisis</p>

          <select className="select-input" value={jenisKambing} onChange={e => setJenisKambing(e.target.value)}>
          ...
          <div className="check-row">
            <input type="checkbox" id="chk-kurban" checked={cekKurban} onChange={e => setCekKurban(e.target.checked)} />
            <label htmlFor="chk-kurban">Cek kelayakan hewan kurban</label>
          </div>
          <div className="check-row" style={{ marginBottom: 16 }}>
            <input type="checkbox" id="chk-harga" checked={cekHarga} onChange={e => setCekHarga(e.target.checked)} />
            <label htmlFor="chk-harga">Estimasi rentang harga jual</label>
          </div>

          <button className="btn-primary" onClick={mulaiAnalisis} disabled={!imageBase64 || loading}>
        ...
        <div className="card" style={{ minHeight: 400 }}>
          ...
          {tokenHabis && (
            <div className="alert alert-red" style={{ lineHeight: 1.7 }}>
              <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}> Kuota Limit Harian Habis</div>
              <div>Limit penggunaan harian AI sudah tercapai.</div>
              <div style={{ marginTop: 10, padding: '10px 12px', background: 'rgba(0,0,0,0.05)', borderRadius: 8 }}>
                 <strong>Reset otomatis:</strong><br />
                {tokenPesan}
              </div>
            </div>
          )}

          {hasil && <HasilAnalisis data={hasil} cekKurban={cekKurban} cekHarga={cekHarga} />}
        </div>
      </div>
    </div>
  )
